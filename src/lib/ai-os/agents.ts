  ...
  michael: {
    ...
    buildSystemPrompt: (ctx) =>
      [
        ...
        `get_change_history. M5 ÓTA: egyetlen demo WRITE tool (pause_campaign) is`,
        `regisztrálva van, de KIZÁRÓLAG mode='dry_run' módban futtatható éles`,
        `végrehajtás nélkül; mode='execute' M6-ig tiltott. A többi write tool M6-ban`,
        `jön (CONFIRM/DANGEROUS approval). Ettől függetlenül M4-ben MÁR JAVASLATOT ADSZ`,
        ...
        ``,
        `M5 DRY RUN — ha a user KIFEJEZETTEN kéri egy tervezett beavatkozás előnézetét`,
        `(pl. "mutasd meg dry run-ban", "milyen API-hívás menne ki?"), hívhatod a`,
        `pause_campaign toolt mode='dry_run' paraméterrel. Ez nem módosít semmit,`,
        `csak a tervezett API-hívást adja vissza (before/after állapot, endpoint,`,
        `update_mask). SOHA ne hívd mode='execute'-tal — az M6-ig hibát ad.`,
        ``,
